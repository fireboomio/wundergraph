...
func (t *ApiTransport) roundTrip(request *http.Request, buf *bytes.Buffer) (res *http.Response, err error) {
	...
	if isUpgradeRequest || t.enableStreamingMode || !logging.NoneStreamContentType(res.Header) {
		if t.debugMode {
			fmt.Printf("\n\n--- DebugTransport ---\n\nRequest:\n\n%s\n\nDuration: %d ms\n\n--- DebugTransport\n\n",
				string(requestDump),
				duration,
...
func (t *ApiTransport) internalGraphQLRoundTrip(request *http.Request) (res *http.Response, err error) {
	...
	if userBytes := authentication.UserBytesFromContext(request.Context()); userBytes != nil {
		if requestBody, err = jsonparser.Set(requestBody, userBytes, "__wg", "user"); err != nil {
			return nil, err
		}
	}
...
func (t *ApiTransport) handleOnResponseHook(r *http.Response, metaData *OperationMetaData, buf *bytes.Buffer) (*http.Response, error) {
	...
	if userBytes := authentication.UserBytesFromContext(r.Request.Context()); userBytes != nil {
		hookData, _ = jsonparser.Set(hookData, userBytes, "__wg", "user")
	}
