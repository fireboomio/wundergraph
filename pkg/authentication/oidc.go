...
func NewOpenIDConnectCookieHandler(url string, log *zap.Logger) *OpenIDConnectCookieHandler {
...
func (h *OpenIDConnectCookieHandler) authorize(_ context.Context, w http.ResponseWriter, r *http.Request, provider *oidc.Provider, config *OpenIDConnectConfig) {
	...
	redirectURI = h.url + redirectPath
	...
	cookieDomain = sanitizeDomain(getHostFromUrl(h.url))
...
func (h *OpenIDConnectCookieHandler) authorizationCallback(ctx context.Context, w http.ResponseWriter, r *http.Request, provider *oidc.Provider, config *OpenIDConnectConfig, hooks Hooks) {
	...
	redirect = h.url + "/auth/cookie/user"
	http.Redirect(w, r, redirect, http.StatusFound)
}
...
func (c *OpenIDConnectConfiguration) Validate(cookieBased bool) error {
...
func introspectionOpenIDConnectProvider(issuer string, client *http.Client, cookieBased bool) (config *OpenIDConnectConfiguration, err error) {
	if _, err = url.ParseRequestURI(issuer); err != nil {
		err = fmt.Errorf("OIDC issuer must be a valid URL: %w", err)
		return
	}

	introspectionURL := strings.TrimSuffix(issuer, "/") + "/.well-known/openid-configuration"
	req, err := http.NewRequest(http.MethodGet, introspectionURL, nil)
	if err != nil {
		err = fmt.Errorf("could not request OIDC configuration: %w", err)
		return
	}
	resp, err := client.Do(req)
	if err != nil {
		err = fmt.Errorf("could not fetch OIDC configuration: %w", err)
		return
	}
	if resp.StatusCode != http.StatusOK {
		err = fmt.Errorf("OIDC configuration returned an HTTP error code: %d", resp.StatusCode)
		return
	}
	...
	if err = dec.Decode(&config); err != nil {
		err = fmt.Errorf("could not decode OIDC configuration: %w", err)
		return
	}
	if err = config.Validate(cookieBased); err != nil {
		err = fmt.Errorf("invalid OIDC configuration: %w", err)
		return
	}
...
func NewOpenIDConnectProvider(issuer string, clientID string, clientSecret string, opts *OpenIDConnectProviderOptions) (*OpenIDConnectProvider, error) {
	...
	config, err := introspectionOpenIDConnectProvider(issuer, opts.httpClient(), true)
	if err != nil {
		return nil, err
	}
...
func (p *OpenIDConnectProvider) disconnectDefault(ctx context.Context, user *User) (*OpenIDDisconnectResult, error) {
	...
	return &OpenIDDisconnectResult{logoutURL}, nil
}
