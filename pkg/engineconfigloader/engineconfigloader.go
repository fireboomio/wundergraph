...
func (d *DefaultFactoryResolver) onWsConnectionInitCallback(dataSourceID string) *graphql_datasource.OnWsConnectionInitCallback {
	var callback graphql_datasource.OnWsConnectionInitCallback = func(ctx context.Context, url string, header http.Header) (json.RawMessage, error) {
		...
		if userBytes := authentication.UserBytesFromContext(ctx); userBytes != nil {
...
func (l *EngineConfigLoader) Load(engineConfig *wgpb.EngineConfiguration, wgServerUrl string) (*plan.Configuration, error) {
	...
	for _, configuration := range engineConfig.FieldConfigurations {
		...
		for _, argumentConfiguration := range configuration.ArgumentsConfiguration {
			...
			case wgpb.ArgumentRenderConfiguration_RENDER_ARGUMENT_AS_JSON_VALUE:
				arg.RenderConfig = plan.RenderArgumentAsJSONValue
			}
	...
	database.ResetPrismaCache()
	database.ResetPlannerConfigCache()
	oas_datasource.ResetPlannerConfigCache()
	for _, in := range engineConfig.DatasourceConfigurations {
		...
		case wgpb.DataSourceKind_REST:
			...
			restConfig := oas_datasource.Configuration{
				Fetch: oas_datasource.FetchConfiguration{
					...
					RequestContentType:  in.CustomRest.Fetch.RequestContentType,
					ResponseContentType: in.CustomRest.Fetch.ResponseContentType,
				},
				Subscription: oas_datasource.SubscriptionConfiguration{
					Enabled:                 in.CustomRest.Subscription.Enabled,
					...
					DoneData:                in.CustomRest.Subscription.DoneData,
				},
				DatasourceName:         in.Id,
				DefaultTypeName:        in.CustomRest.DefaultTypeName,
				StatusCodeTypeMappings: typeMappings,
				RequestRewriters:       in.CustomRest.RequestRewriters,
				ResponseRewriters:      in.CustomRest.ResponseRewriters,
				ResponseExtractor:      in.CustomRest.ResponseExtractor,
			}
			oas_datasource.AddPlannerConfigCache(in.RootNodes, restConfig)
		case wgpb.DataSourceKind_STATIC:
		...
		case wgpb.DataSourceKind_GRAPHQL:
			...
			out.Custom = graphql_datasource.ConfigJson(graphql_datasource.Configuration{
				...
				Subscription: graphql_datasource.SubscriptionConfiguration{
					...
					SSEMethodPost: in.CustomGraphql.Subscription.UseSSE,
				},
		...
		case wgpb.DataSourceKind_POSTGRESQL,
			...
			database.StorePlannerConfig(in.RootNodes, config)
		default:
			continue
		}
	...
	outConfig.DisallowParallelFunc = disallowParallelFetch
	return &outConfig, nil
}

var disallowParallelFetchDirective []string

func AddDisallowParallelFetchDirective(name string) {
	disallowParallelFetchDirective = append(disallowParallelFetchDirective, name)
}

func disallowParallelFetch(operation *ast.Document) bool {
	return slices.ContainsFunc(operation.Directives, func(directive ast.Directive) bool {
		directiveName := operation.Input.ByteSliceString(directive.Name)
		return slices.Contains(disallowParallelFetchDirective, directiveName)
	})
}
...
func buildFetchUrl(url, baseUrl, path string, hooksServerUrl string) string {
	...
	baseUrl, path = strings.TrimSuffix(baseUrl, "/"), strings.TrimPrefix(path, "/")
	if baseUrl == "" {
		return path
	}
