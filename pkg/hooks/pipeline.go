...
func newPipeline(config PipelineConfig) pipeline {
	...
	return pipeline{
		...
		GeneratedVariables:     config.GeneratedVariables,
		ResolveConfig: ResolveConfiguration{
			...
			Mock: hooksConfig.GetMockResolve().GetEnabled(),
		},
		PostResolveConfig: PostResolveConfiguration{
...
func (p *pipeline) updateContextHeaders(ctx *resolve.Context, headers map[string]string) {
	...
	if cr, ok := clientRequest.(*http.Request); ok {
		for name := range headers {
			cr.Header.Set(name, headers[name])
		}
	}
}
...
func (p *pipeline) PreResolve(ctx *resolve.Context, w http.ResponseWriter, r *http.Request) (*Response, error) {
	...
	if p.ResolveConfig.Pre {
		...
		resp, err = p.client.DoOperationRequest(ctx.Context, p.operation.Path, PreResolve, hookData, payloadBuf)
		if err != nil {
			return nil, err
		}
	...
	if p.ResolveConfig.MutatingPre {
		...
		resp, err = p.client.DoOperationRequest(ctx.Context, p.operation.Path, MutatingPreResolve, hookData, payloadBuf)
		if err != nil {
			return nil, err
		}
		...
		data = resp.Input
		if p.GeneratedVariables != nil {
			_ = jsonparser.ObjectEach(p.GeneratedVariables, func(key []byte, value []byte, valueType jsonparser.ValueType, offset int) error {
				if valueType == jsonparser.String {
					value = p.GeneratedVariables[offset-len(value)-2 : offset]
				}
	...
	if p.ResolveConfig.Mock {
		...
		resp, err := p.client.DoOperationRequest(ctx.Context, p.operation.Path, MockResolve, hookData, payloadBuf)
		if err != nil {
			return nil, err
		}
		if done := p.handleHookResponse(ctx, w, MockResolve, resp); done {
			return &Response{
				Done: true,
			}, nil
		}
	...
	if p.ResolveConfig.Custom {
		...
		resp, err = p.client.DoOperationRequest(ctx.Context, p.operation.Path, CustomResolve, hookData, payloadBuf)
		if err != nil {
			return nil, err
		}
...
func (p *pipeline) PostResolve(ctx *resolve.Context, w http.ResponseWriter, r *http.Request, responseData []byte) (*Response, error) {
	...
	if p.PostResolveConfig.Post {
		...
		resp, err := p.client.DoOperationRequest(ctx.Context, p.operation.Path, PostResolve, postResolveData, payloadBuf)
		if err != nil {
			return nil, err
		}
		if done := p.handleHookResponse(ctx, w, PostResolve, resp); done {
			return &Response{
				Done: true,
			}, nil
		}
	...
	if p.PostResolveConfig.MutatingPost {
		...
		resp, err := p.client.DoOperationRequest(ctx.Context, p.operation.Path, MutatingPostResolve, mutatingPostData, payloadBuf)
		if err != nil {
			return nil, err
		}
		if done := p.handleHookResponse(ctx, w, MutatingPostResolve, resp); done {
			return &Response{
				Done: true,
			}, nil
		}
...
func deepCopyVariables(variables []byte) []byte {
	copyVariables := make([]byte, len(variables))
	copy(copyVariables, variables)
...
func (p *SynchronousOperationPipeline) Run(ctx *resolve.Context, w http.ResponseWriter, r *http.Request, buf *bytes.Buffer) (*Response, error) {
	...
	if err != nil {
		return nil, errors.WithMessage(err, "preResolve hooks failed")
	}
	...
	// variables在后续ResolveGraphQLResponse中函数中会被改动
	...
	if err != nil {
		return nil, errors.WithMessage(err, "ResolveGraphQLResponse failed")
	}
	...
	if p.postResolveTransformer != nil {
		...
		if err != nil {
			return nil, errors.WithMessage(err, "postResolveTransformer failed")
		}
	} else {
		response = buf.Bytes()
	}
	...
	if err != nil {
		return nil, errors.WithMessage(err, "postResolve hooks failed")
	}

	if r.URL.Query().Has(paramStripComponent) {
		postResolveResp.Data, _, _, _ = jsonparser.Get(postResolveResp.Data, "data")
	}
...
const paramStripComponent = "strip-component"
