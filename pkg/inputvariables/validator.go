...
func NewValidator(schema string, disableVerboseErrors bool) (*Validator, error) {
	...
	err := jsonIterator.Unmarshal([]byte(schema), &validator.schema)
	if err != nil {
		return nil, err
	}
...
func (v *Validator) ValidateInputVariables(ctx context.Context, log *zap.Logger, variables []byte, w http.ResponseWriter) bool {
	var buf bytes.Buffer
	valid, err := v.Validate(ctx, variables, &buf)
	if err != nil {
