...
func (s *StringInterpolator) configure(schema *Schema, path ...string) {
	typeStr := schema.typeString()
	if len(path) > 0 && s.PropertiesTypeMap != nil {
		s.PropertiesTypeMap[strings.Join(path, ".")] = typeStr
	}
	...
	case "array":
		...
		s.configure(schema.Items, append(path, "[]")...)
	case "":
		s.hasInterpolations = true
	}
}
