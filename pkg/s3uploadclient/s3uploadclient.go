...
const (
	...
	HeaderUploadProfile = "X-Upload-Profile"
)
...
func (s *S3UploadClient) uploadProfile(r *http.Request) (profileName string, profile *preparedProfile, err error) {
	if profileName = r.Header.Get(HeaderUploadProfile); profileName != "" {
...
func (s *S3UploadClient) uploadToS3(ctx context.Context, r *http.Request, part *multipart.Part) (*minio.UploadInfo, error) {
	...
	tmp, err := os.CreateTemp("", fmt.Sprintf("fireboom-upload.*%s", extension))
	if err != nil {
		return nil, err
	}
	defer func() {
...
func (s *S3UploadClient) preUpload(ctx context.Context, r *http.Request, part *multipart.Part, tempFile *os.File, fileSize int64) (string, error) {
	...
	if profile != nil {
		...
		if profile.UsePreUploadHook {
			...
			data, err := hookData(buf.Bytes(), r, s.name, part, part.FileName(), fileSize, nil)
			if err != nil {
	...
	if dir := r.URL.Query().Get("directory"); dir != "" {
		...
		fileKey = filepath.ToSlash(filepath.Join(dir, fileKey))
	}
	return fileKey, nil
}
...
func (s *S3UploadClient) postUpload(ctx context.Context, r *http.Request, part *multipart.Part, info *minio.UploadInfo, uploadError error) error {
	...
	if profile != nil && profile.UsePostUploadHook {
		...
		data, err := hookData(buf.Bytes(), r, s.name, part, info.Key, fileSize, uploadError)
		if err != nil {
...
func fileMetadataFromRequest(r *http.Request) string {
	return r.Header.Get(HeaderMetadata)
}

func hookData(buf []byte, r *http.Request, provider string, part *multipart.Part, filename string, fileSize int64, uploadError error) ([]byte, error) {
	buf = buf[:0]
	buf = append(buf, []byte(`{"__wg":{}}`)...)
	if userBytes := authentication.UserBytesFromContext(r.Context()); userBytes != nil {
		var err error
		if buf, err = jsonparser.Set(buf, userBytes, "__wg", "user"); err != nil {
			return nil, err
		}
	}
	if part != nil {
